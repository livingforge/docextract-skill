...
- specdb ツール一式は `.claude/skills/specdb/scripts/`（`--root` 省略時は自動で `./.specdb` を使う）
...
```bash
python .claude/skills/specdb/scripts/engine.py     # 検証 + 集計。error があれば exit 1
```
...
```bash
python .claude/skills/specdb/scripts/visualize.py  # .specdb/out/specdb.html（対話型グラフ）
python .claude/skills/specdb/scripts/generate.py   # .specdb/documents/ に定義された設計書
```
