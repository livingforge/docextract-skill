...
（`.claude/skills/specdb/scripts/requirements.txt`）:
...
手動でコピーしたい場合は雛形 `.claude/skills/specdb/scaffold/` を使う:

```bash
cp -r .claude/skills/specdb/scaffold .specdb     # サンプル入り。items/・relations/ は削除可
```
...
コマンドが未 install の環境では `python .claude/skills/specdb <サブコマンド>` でも同じ。
