...
"""横断集計 — 複数プロジェクトの仕様データを標準パック基準でまとめる
    ...
    python specdb/aggregate.py <root1> <root2> ...        # Markdown をstdoutへ
    python specdb/aggregate.py <root...> --out 台帳.md      # ファイルへ
    python specdb/aggregate.py <root...> --type data-item  # 特定種別だけ
...
def main() -> int:
    ...
    if len(roots) < 1:
        print("使い方: specdb aggregate <root1> <root2> ... [--type 種別] [--out file]",
              file=sys.stderr)
        return 2
